...
public class Adafruit extends BluetoothGattCallback implements BluetoothAdapter.LeScanCallback{

    // UUIDs for UART service and associated characteristics.
    public static UUID UART_UUID = UUID.fromString("6E400001-B5A3-F393-E0A9-E50E24DCCA9E");
    public static UUID TX_UUID   = UUID.fromString("6E400002-B5A3-F393-E0A9-E50E24DCCA9E");
    public static UUID RX_UUID   = UUID.fromString("6E400003-B5A3-F393-E0A9-E50E24DCCA9E");
    public ScanCallback SCAN_FAILED_ALREADY_STARTED;
    ...
    public interface Callback {
        public void onConnected(Adafruit uart);
        public void onConnectFailed(Adafruit uart);
        public void onDisconnected(Adafruit uart);
        public void onDeviceInfoAvailable();
        public void onReceive(Adafruit uart, BluetoothGattCharacteristic rx);
        public void onDeviceFound(BluetoothDevice device);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public Adafruit(Context context) {
        super();
        ...
        this.SCAN_FAILED_ALREADY_STARTED = new ScanCallback() {
            @Override
    ...
    public void stopScan() {
        if (adapter != null) {
            scanner.stopScan(SCAN_FAILED_ALREADY_STARTED);
        }
    }
    ...
    public void startScan() {
        if (adapter != null) {
            scanner.startScan(SCAN_FAILED_ALREADY_STARTED);
        }
    }
    ...
    private void notifyOnConnected(Adafruit uart) {
    ...
    private void notifyOnConnectFailed(Adafruit uart) {
        for (Callback cb : callbacks.keySet()) {
            if (cb != null) {
    ...
    private void notifyOnDisconnected(Adafruit uart) {
        for (Callback cb : callbacks.keySet()) {
            if (cb != null) {
    ...
    private void notifyOnReceive(Adafruit uart, BluetoothGattCharacteristic rx) {
        for (Callback cb : callbacks.keySet()) {
            if (cb != null ) {
